...
<template>
  <div class="relative w-full max-w-[380px]">
    ...
    <div
      ...
      class="absolute left-0 right-0 top-14 bg-bg-surface border border-border-default rounded-md shadow-lg max-h-80 overflow-y-auto z-50 p-2 space-y-2"
      role="listbox"
    >
      ...
      <div v-if="filteredOperators.length > 0">
        ...
        <button
          ...
          class="w-full text-left px-3 py-2 text-sm rounded hover:bg-bg-subdued flex items-center gap-2 touch-target-min"
          role="option"
        >
      ...
      <div v-if="filteredCities.length > 0">
        ...
        >
          ...
          <span class="font-medium text-text-primary">{{ c }}</span>
          <span class="text-xs text-text-secondary ml-auto">Şehir Haritası</span>
        </button>
      </div>

      <!-- Sonuç Bulunamadı Durumu -->
      <div
        v-if="!isSearching && filteredOperators.length === 0 && filteredCities.length === 0"
      ...
      >
        ...
        <p class="text-[11px] text-text-secondary">
          İlçe veya operatör adı yazarak tekrar deneyin.
        </p>
      </div>
    </div>
  </div>
