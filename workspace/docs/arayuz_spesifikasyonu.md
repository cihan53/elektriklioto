...
> **Doğruluk Kaynakları:** `proje_kapsami.md`, `workspace/docs/teknik_mimari_dokumani.md`, `workspace/docs/tasarim_sistemi.md`, `workspace/docs/ux_akislari.md`, `workspace/docs/ekran_envanteri.md`, `workspace/docs/veri_kaynagi_epdk.md`
...
- **Mobil (`< 640px`):**
  ...
  - İstasyon Detayı: 3 kademeli alt çekmece (`StationDetailSheet` — Kademe 1: Peek 160pt, Kademe 2: Half 380pt, Kademe 3: Full `calc(100vh - 24px)`).
- **Tablet (`640px - 1023px`):**
  - Kenar Boşluğu: 24px (`--spacing-6`).
  - Sütun Sayısı: 2 kolonlu kart ızgarası veya harita üzerinde yüzen sol panel (tek tip **340px sol yüzen sheet**, harita üstünde soldan 24px, üstten 80px, alttan 24px boşluk, `radius-lg`, `shadow-lg`).
...
- Tıklanabilir tüm elemanlar webde en az **44x44 CSS px** (`min-h-[44px] min-w-[44px]`), mobilde (Flutter) istisnasız en az **48x48 pt** (`minTargetSize: 48.0` / `BoxConstraints(minWidth: 48, minHeight: 48)`) fiziksel alana sahip olmak zorundadır. Görsel olarak daha küçük tasarlanan tüm öğeler (örn: 32px kompakt butonlar, 36px filtre çipleri, 24px kapatma ikonları) mobilde şeffaf iç boşlukla (padding/hit-test) bu sınırlara genişletilir.
- Klavye sekme (Tab) odağında 3px kalınlığında `--color-focus-ring` (açıkta `#0066CC`, koyuda `#38BDF8`) odak halkası zorunludur; `outline-offset: 2px`. Odak asla gizlenemez.
- `prefers-reduced-motion` aktif olduğunda tüm geçiş ve animasyonlar anında sıfırlanır: `transition: none !important; animation: none !important;`.
...
- **Kırılım Düzeni:**
  - *Masaüstü (≥ 1024px):* Üstte 64px Header. Altında tam ekran harita. Harita üzerinde sol üstte arama çubuğu ve yatay filtre çubuğu. Pin tıklandığında sol 380px panel kayarak açılır (`transition: 200ms ease-out`).
  - *Tablet (640-1023px):* Üstte arama ve filtreler; pin tıklandığında solda yüzen 340px sheet açılır.
  - *Mobil (< 640px):* Tam ekran harita. Üstte 48px yüzen arama çubuğu (kenarlardan 16px). Arama altında yatay kaydırılabilir filtre hapları. Sağ altta 48x48pt dairesel "Konumuma Git" FAB. Pin tıklandığında alttan 160pt'lik `StationDetailSheet` belirir.

#### Bileşen Yerleşimi (Hiyerarşi)
1. **Arama Çubuğu (`SearchInput`):**
   - Genişlik: Webde 380px, mobilde `calc(100% - 32px)`.
   - Yükseklik: 48px, radius 8px (`--radius-md`), sol ikon `Search` (20px, `--color-text-secondary`).
   - Mikro Kopya (Placeholder): `"İstasyon, ilçe veya operatör ara..."` (Renk: `--color-text-secondary`, `#64748B` açık, `#94A3B8` koyu; WCAG AA uyumlu).
   - Asenkron Durum (`Searching / Loading`): Kullanıcı giriş yaparken (debounce: 250ms), sağ tarafta 20px döner spinner (`--color-primary`) aktifleşir.
   - Temizleme Butonu: Metin varken sağda 20px `X` butonu (dokunma hedefi webde 44x44px, mobilde 48x48pt).
2. **Arama Sonuç Açılır Menüsü (`SearchAutocompleteDropdown`):**
   - Konum: Arama kutusunun hemen altında (top: 56px), zemin `--color-bg-surface`, border 1px solid `--color-border-default`, `shadow-lg`, radius `--radius-md`, max yükseklik 320px (`overflow-y-auto`).
   - Sonuç Maddesi: İlçe veya Operatör adı (`text-body-md`), sağda istasyon sayısı (`text-caption`, `--color-text-secondary`). Yükseklik webde 44px, mobilde 48pt; hover zemin `--color-bg-subdued`.
   - Açılır Menü Boş Durumu (Autocomplete Empty State):
     - İkon: `SearchX` (20px, `--color-text-secondary`).
     - Başlık: `"Sonuç bulunamadı"` (`text-body-md` SemiBold, `--color-text-primary`).
     - Açıklama: `"Aradığınız kriterle eşleşen ilçe veya operatör bulunamadı. Lütfen yazımı kontrol edin veya haritayı kaydırın."` (`text-body-sm`, `--color-text-secondary`).
3. **Filtre Çubuğu (`FilterChips`):**
   - Yatay kaydırılabilir liste (`gap-2`, padding: 8px 16px, klavye ile sekmeleyebilir).
   - Chip 1: `"Operatörler"` (Açılır menü tetikler; 179 markadan çoklu seçim; seçilince `"Operatör (2)"` formatına döner; `aria-pressed="false|true"`).
   - Chip 2: `"Halka Açık"` (İki durumlu toggle; default pasif, basılınca aktif; `aria-pressed="false|true"`).
   - Chip 3: `"Hızlı Şarj (DC)"` (> **VERİ YOK:** Faz 1'de kilitli pasif, `aria-disabled="true"`; dokunulduğunda toast çıkar).
   - Chip 4: `"Boş Soketler"` (> **VERİ YOK:** Faz 1'de kilitli pasif, `aria-disabled="true"`; dokunulduğunda toast çıkar).
   - Klavye Odak: Çip üzerinde sekmelendiğinde 3px `--color-focus-ring` odak halkası görünür.
   - Dokunma Alanı: Webde min 44x44px, mobilde min 48x48pt.
4. **Konum FAB (`LocationButton` — Yalnızca Mobil):**
...
5. **Harita Tuvali (`VectorMapCanvas`):**
   - BBox hareket bitişinde (debounce: 300ms) sınır koordinatları iletilir (`GET /api/v1/stations`). Ham GPS sunucuya iletilmez.
   - Zoom < 11: Küme daireleri (`cluster_count`).
   - Zoom ≥ 11: 40x48px damla pinler (operatör rengi ve ikonu).

#### Arayüz Durumları
- **Varsayılan:** Pinler ve harita karoları yüklü, 60 FPS etkileşim.
- **Yükleniyor:** Sağ üst köşede 24px spinner (`--color-primary`); mevcut harita pinleri ekranda kalır.
- **Boş Durum (Bölgede İstasyon Yok):** Haritanın üst orta kısmında yüzen kapsül rozet (`shadow-md`, dolgu: 8px 16px):
...
- **Hata Durumu:**
  ...
  - Mobil: Ağ koptuğunda üstte sarı/gri bildirim şeridi açılır: `"Çevrimdışı Mod — Son bilinen istasyonlar gösteriliyor."` (Hive önbelleği).
...
- **Hedef:** İstasyonun adres, operatör ve EPDK sicil bilgilerini göstermek; CPO uygulamasına derin bağlantı (mobil) veya operatör web sitesi/QR köprüsü (web masaüstü) sunmak.
- **Kırılım Düzeni:**
  - *Masaüstü (≥ 1024px):* Sol sabit 380px panel; dikey kaydırılabilir (`overflow-y-auto`), dolgu 24px (`--spacing-6`). Kapatma butonu sağ üstte 32x32px (dokunma alanı webde 44x44px).
  - *Tablet (640-1023px):* 340px sol yüzen sheet (`shadow-lg`, `radius-lg`).
  - *Mobil (< 640px):* 3 Kademeli `StationDetailSheet`.
    - Kademe 1 (Peek — 160pt): Çekmece tutamacı, Operatör adı, İstasyon adı, Mesafe rozeti (GPS yoksa Hizmet Şekli rozeti), Birincil CTA.
    - Kademe 2 (Half — 380pt): Hizmet şekli rozeti, Eksik veri rozetleri, Açık Adres, Yol Tarifi butonu.
    - Kademe 3 (Full — `calc(100vh - 24px)`): EPDK Sicil No, Arıza Bildir CTA, Topluluk Veri Ekle CTA, EMP yasal beyanı.
...
2. **Başlık ve Operatör Bloğu:**
   - Operatör Logosu/Adı: 24px logo + `text-body-sm` (14px) `--color-text-secondary` operatör ticari unvanı.
   - İstasyon Adı: `text-h3` (20px SemiBold), `--color-text-primary`.
   - EPDK Sicil Rozeti: `text-mono` (13px Medium), zemin `--color-bg-subdued`, metin rengi: `--color-text-secondary` (`#475569` açık, `#94A3B8` koyu; zemin üzerindeki kontrast oranı ≥ 4.5:1 WCAG AA), metin: `"EPDK: ŞRJ/xxxx"`.
3. **Mesafe ve Konum İzni Davranışı (Mobil Peek Çekmecesi Kuralı):**
   - *Konum İzni Varsa:* Sağ üstte yeşil/mavi rozet: `~{X} km` (Cihaz in-memory hesaplar).
   - *Konum İzni Yoksa veya GPS Kapalıysa:* Mesafe rozeti tamamen GİZLENİR; yerine `Hizmet Şekli` rozeti (`"Halka Açık"`) sola yaslanarak gösterilir. Arayüzde boşluk veya çökme oluşmaz.
4. **Durum ve Hizmet Rozetleri (Yatay Esnek Blok):**
   - Rozet 1 (Hizmet Şekli): `"Halka Açık"` (yeşil zemin) veya `"Özel / Kısıtlı"` (sarı zemin).
   - Rozet 2 (Arıza Varsa — 3+ Doğrulanmış İhbar): `"Arıza Bildirildi"` (kırmızı zemin `--color-danger-subdued`, metin `--color-danger-on-subdued: #FEE2E2`, kontrast: 8.20:1, 14px uyarı ikonu).
5. **Eksik Veri Bölümü (> **VERİ YOK:** Faz 1 Kuralı):**
   ...
   - Rozet: Standart Gri Rozet (`--color-missing-bg`, metin: `--color-missing-text`):
     ...
     - Bitişik Buton (CTA): `[+ Bilgi Ekle]` (Küçük ikincil buton, yükseklik 32px; dokunma alanı webde ≥ 44x44 CSS px, mobilde `BoxConstraints(minWidth: 48, minHeight: 48)` ile en az 48x48 pt; SCR-07'yi tetikler).
   - Tarife Satırı: `"Tarife: Operatör Verisi Bekleniyor"`.
   - Doluluk Satırı: `"Canlı Doluluk: Canlı durum verisi henüz açılmadı"`.
6. **Adres ve Konum Bilgisi:**
   - İkon: `MapPin` (18px, `--color-text-secondary`).
   - Metin: Açık adres, İlçe / İl (`text-body-md`, `--color-text-secondary`).
7. **Aksiyon Buton Grubu (Platform Ayrımı):**
   - **Mobil İstemci (Flutter):**
     - Birincil CTA Butonu (`ButtonPrimary`, 48pt, tam genişlik):
       - Metin: `"Operatörde Aç / Şarja Başla"`
       - Davranış: CPO uygulaması varsa doğrudan şemayı (`zes://station/{no}`) tetikler; yoksa mağazaya yönlendirir; desteklenmiyorsa istasyon kodunu panoya kopyalar (`clipboard_fallback`), toast verir ve linki açar.
   - **Web Masaüstü (Nuxt 3 Masaüstü Tarayıcı):**
     - Birincil CTA Butonu (`ButtonPrimary`, 48px, tam genişlik):
       - Metin: `"Operatör Web Sitesine Git ↗"`
       - Davranış: Operatörün resmî web sitesini yeni sekmede açar.
       - Buton Altı Mikro Kopya: `"Masaüstü ortamında doğrudan şarj başlatılamaz; operatör web sitesine gidebilir veya QR ile telefona aktarabilirsiniz."` (`text-caption`, 11px, `--color-text-secondary`).
   - **İkincil Buton Grubu (2 Kolon, yükseklik webde 44px, mobilde 48pt):**
     - Sol Buton: `[Yol Tarifi Al]` (Harici Apple Maps / Google Maps açar).
     - Sağ Buton (Web): `[Telefona Aktar (QR)]` (SCR-05 QR modalını açar).
     - Sağ Buton (Mobil): `[Favoriye Ekle]` (Yıldız ikonu; lokal Hive/localStorage).
   - **Arıza Bildir Butonu (Metin Bağlantı Butonu, dokunma alanı webde 44x44px, mobilde 48x48pt):**
     - Metin: `"İstasyonla ilgili bir sorun mu var? Arıza Bildir"`.
8. **Tazelik ve Yasal EMP Beyanı (Dipnot Bloğu):**
   - Veri Kaynak Damgası: `"Veri Kaynağı: EPDK Sicil Kaydı (Eylül 2026)"` (`text-body-sm`, `--color-text-secondary`). *EPDK veri setinde istasyon bazlı güncelleme tarihi olmadığından dinamik göreceli tarih ("X gün önce") kesinlikle kullanılmaz; tohumlama referansı sabitlenir.*
   - Yasal Uyarı Metni: `"elektriklioto.com lisanslı şarj operatörü değildir. Şarj başlatma ve faturalandırma ilgili operatörün sorumluluğundadır."` (`text-caption`, 11px, `--color-text-secondary`).

#### Arayüz Durumları
- **Varsayılan:** EPDK doğrulanmış alanlar görünür, eksik alanlar gri rozetli.
...
3. **İstasyon Kart Listesi (`StationSummaryCard`):**
   ...
   - Durum Rozeti: `"Operatör Verisi Bekleniyor"` (Standart gri rozet).
   - Aksiyon: `[Detayları İncele →]` bağlantısı (Dokunma alanı webde ≥ 44x44px).
4. **Sayfalama (Pagination):**
   - Her sayfada 24 istasyon kartı. Altta `[Önceki]` ve `[Sonraki]` numaralandırılmış butonlar (dokunma hedefi ≥ 44x44px).
...
- **Kırılım Düzeni:**
  ...
  - *Tablet (640-1023px):* 2 kolonlu kompakt hero, tek kolon liste.
...
1. **Operatör Başlık Bloğu:**
   ...
   - Lisans Sahibi Unvanı: `sarj_agi_isletmecisi` resmî şirket adı (`text-body-md`, `--color-text-secondary`).
...
3. **Mobil Uygulama Köprüsü:**
   ...
   - Butonlar: `[App Store]` ve `[Google Play]` mağaza rozetleri (dokunma hedefi ≥ 44x44px).
...
- **Boş Durum:** İstasyonu henüz işlenmemiş aktif lisanslarda:
...
- **Hedef:** Masaüstünde incelenen rota veya istasyonun, kullanıcı hesabı açtırmadan ve sunucuda koordinat tutmadan (sıfır KVKK riski) mobil cihaza aktarılması.
- **Kırılım Düzeni:**
  - *Masaüstü:* Ekran ortasında 420px genişliğinde modal (`radius-xl`, `shadow-xl`, dolgu 24px). ESC tuşu veya kapat butonuyla kapanır.
...
1. **Modal Başlığı ve Kapat Butonu:**
   ...
   - Kapat Butonu: Sağ üstte `X` (webde 44x44px dokunma alanı).
...
3. **Yönerge Metni:**
   - `"Telefonunuzun kamerasıyla QR kodu okutun. Rota ve seçili istasyonlar elektriklioto.com mobil uygulamasında anında açılacaktır."` (`text-body-md`, `--color-text-secondary`).
4. **Alternatif Paylaşım Aksiyonu:**
   - Buton (`ButtonSecondary`, 44px): `[Bağlantıyı Kopyala]` (Panoya kopyalar; toast: `"Bağlantı kopyalandı!"`).

#### Arayüz Durumları
- **Üretim:** Kod < 100ms içinde yerel olarak üretilir; gecikme algılanmaz.
...
- **Mobil Kamera İzni Yok:** Ekranda uyarı: `"QR okutabilmek için kamera izni gereklidir."` + `[Ayarları Aç]` butonu (dokunma hedefi ≥ 48x48pt).
...
#### Bileşen Yerleşimi
1. **Konum İzni Kapalı / GPS Devre Dışı Durumu (Zorunlu Güvenlik Kuralı):**
   - Cihazda donanım GPS'i kapalıysa veya uygulamanın konum izni engellenmişse formun en başında sarı uyarı kartı açılır:
     - İkon: `AlertCircle` (20px, `--color-warning`).
     - Metin: `"Arıza doğrulaması için konum izni gereklidir. İstasyonun yanında olduğunuzu doğrulamadan bildirim gönderilemez."`
     - Aksiyon Butonu: `[Konum İznini Aç / Ayarlar]` (Cihaz ayarlarını tetikler; dokunma hedefi webde ≥ 44x44px, mobilde ≥ 48x48pt).
     - Form Kuralı: `Gönder` butonu KİLİTLİ PASİF (`Disabled`) tutulur. Sonsuz spinner gösterilmez.
2. **Modal Başlığı:**
   - Başlık (H3): `"Arıza / Durum Bildir"`
   - İstasyon Adı: `{istasyon_adi}` (`text-body-sm`, `--color-text-secondary`).
3. **Mesafe Doğrulama Göstergesi (Proximity Status — Konum İzni Varsa):**
...
4. **Sorun Türü Seçim Listesi (Tekli Seçim Radyo Kartları):**
...
5. **Açıklama Giriş Alanı (Opsiyonel):**
   - Textarea, yükseklik 80px, max 140 karakter, border `--color-border-default`.
   - Placeholder: `"Ek detay ekleyin (örn: Ekran donmuş durumda)..."`
6. **Gönder Butonu (`ButtonPrimary`, webde 48px, mobilde 48pt):**
   - Metin: `"Bildirimi Gönder"`
   - Kural: GPS kapalıysa, mesafe > 50m ise veya sorun seçilmemişse buton `Disabled` durumdadır.
...
- **Kırılım Düzeni:** Webde 440px merkez modal; mobilde alttan açılan form çekmecesi.
...
3. **Tahmini Şarj Gücü Seçimi (Hap Butonlar):**
   - Seçenekler: `[22 kW]` `[60 kW]` `[120 kW]` `[180 kW+]` `[Bilmiyorum]` (Her buton dokunma alanı webde ≥ 44x44px, mobilde ≥ 48x48pt).
...
5. **Gönder Butonu (`ButtonPrimary`, webde 48px, mobilde 48pt):**
...
2. **İstasyon Kart Listesi:**
   ...
   - Aksiyonlar:
     ...
     - Silme (Swipe / Buton): Kartı sola kaydırarak veya çöp kutusu ikonuyla silme (dokunma hedefi webde ≥ 44x44px, mobilde ≥ 48x48pt).
     - Şarja Başla Butonu (Kompakt 36px buton; dokunma alanı webde ≥ 44x44 CSS px, mobilde `BoxConstraints(minWidth: 48, minHeight: 48)` ile en az 48x48 pt): Operatör derin bağlantısını tetikler.

#### Arayüz Durumları
- **Varsayılan:** Kayıtlı istasyonlar listelenir.
- **Boş Durum (Favori Yok):**
  - İkon: `StarOff` (48px, `--color-text-secondary`).
  ...
  - Buton (`ButtonPrimary`, webde 44px, mobilde 48pt): `[İstasyonları Keşfet]` (Haritaya yönlendirir).
...
- **Kırılım Düzeni:** Masaüstünde modal veya sayfa; mobilde Sekme 4 tam ekran liste.

#### Bileşen Yerleşimi
1. **Bölüm 1: Görünüm ve Tema:**
   - Başlık: `"Görünüm"`
   - Seçenek Grubu (3 Seçenekli Segment Buton, yükseklik webde 44px, mobilde 48pt):
     - `[ Sistem ]` | `[ Açık ]` | `[ Koyu ]`
...
2. **Bölüm 2: Depolama ve Çevrimdışı Harita:**
   ...
   - Buton (`ButtonSecondary`, webde 44px, mobilde 48pt): `[Önbelleği Temizle]` (Onay diyaloğu açar: `"Önbellek temizlensin mi?"`).
...
4. **Uygulama Sürümü:**
   - Metin: `"elektriklioto.com v1.0.0-faz1 (build 102)"` (`text-body-sm`, `--color-text-secondary`, ortalanmış).
...
4. **Hızlı Bağlantılar ve Aksiyon:**
   - Birincil Buton (`ButtonPrimary`, webde 48px, mobilde 48pt): `[Haritayı Aç ve Keşfet]`
   - Popüler Dizin Bağlantıları (Dokunma alanı webde ≥ 44x44px, mobilde ≥ 48x48pt):
...
| Durum / Konum | Standart Mikro Kopya Metni | Görsel Stil / Token ve Dokunma Kuralı |
|---|---|---|
| **Eksik Soket / Güç Rozeti** | `"Operatör Verisi Bekleniyor"` | Zemin: `--color-missing-bg`, Metin: `--color-missing-text`, İkon: `HelpCircle` 14px |
| **Eksik Veri Katkı Butonu** | `"[+ Bilgi Ekle]"` | Küçük ikincil buton, 32px görsel yükseklik; dokunma hedefi webde ≥ 44x44px, mobilde `BoxConstraints(minWidth: 48, minHeight: 48)` ile en az 48x48 pt |
| **Eksik Canlı Tarife** | `"Tarife: Operatör Verisi Bekleniyor"` | `text-body-md`, `--color-text-secondary` |
| **Eksik Canlı Doluluk** | `"Canlı durum verisi henüz açılmadı"` | `text-body-sm`, `--color-text-secondary` |
...
| **Zorunlu EMP Yasal Uyarısı** | `"elektriklioto.com lisanslı şarj operatörü değildir. Şarj başlatma ve faturalandırma ilgili operatörün sorumluluğundadır."` | `text-caption` (11px), `--color-text-secondary`, İstasyon detay kartı tabanı |
| **Masaüstü Web Şarj Yönlendirme** | `"Masaüstü ortamında doğrudan şarj başlatılamaz; operatör web sitesine gidebilir veya QR ile telefona aktarabilirsiniz."` | `text-caption` (11px), `--color-text-secondary`, Masaüstü buton altı |
...
| **Konum İzni Kapalı Uyarısı** | `"Arıza doğrulaması için konum izni gereklidir. İstasyonun yanında olduğunuzu doğrulamadan bildirim gönderilemez."` | Sarı uyarı kartı (`--color-warning-surface`), İkon: `AlertCircle` |
| **Çevrimdışı Mod Şeridi** | `"Çevrimdışı Mod — Son bilinen istasyon verileri gösteriliyor."` | Üst bant, Sarı/Gri zemin, İkon: `WifiOff` |
| **Boş Arama Sonucu** | `"Aramanızla eşleşen istasyon bulunamadı. Farklı bir ilçe veya operatör deneyin."` | Ortalı metin, `text-body-md`, `--color-text-secondary` |
...
- [ ] **Dokunma Hedefi Doğrulaması:** Web'de hiçbir buton/input 44x44 CSS px'den, mobilde 48x48 pt'den (`BoxConstraints(minWidth: 48, minHeight: 48)`) küçük olamaz.
- [ ] **WCAG 2.1 AA Kontrast Testi:** Açık ve koyu temalarda tüm metin/zemin kontrast oranları ≥ 4.5:1 (büyük başlıklar ≥ 3.0:1) olmalıdır. Subdued zeminlerde `Text Muted` kullanılamaz; `--color-text-secondary` veya `--color-text-primary` kullanılmalıdır.
- [ ] **Eksik Veri Görünürlüğü:** Soket tipi, güç ve tarife boşken hiçbir sahte veri (örn. "22kW") basılamaz; standart gri "Operatör Verisi Bekleniyor" rozeti ve bitişiğindeki "Bilgi Ekle" CTA'sı eksiksiz render edilmelidir.
...
- [ ] **Klavye ve Ekran Okuyucu Uyumu:** Web kontrollerinde Tab ile gezilebilmeli; 3px odak halkası görünmeli; `aria-pressed`, `aria-disabled` nitelikleri eksiksiz bağlanmalıdır.
