...
| **UAT-03** | Büyükşehir (İstanbul) Zoom 10-12 ve 500+ pin teyidi | İstanbul kümesine tıklandığında zoom 10-12 seviyelerine uçulmalı ve haritaya 500+ istasyon pini düşmelidir. | İstanbul kümesine tıklandı; zoom 11 seviyesine uçuldu ve haritaya tam **544 istasyon pini** (`.station-pin`) başarıyla düştü (Hedef: 500+). | **GEÇTİ** |
| **UAT-04** | Pin tıklama, detay paneli, soket/güç ve derin bağlantı | Pin seçildiğinde yan panel açılmalı; operatör adı, EPDK kodu, eksik veri rozeti ve 'Operatörde Aç' kopyalama/yönlendirme aksiyonu çalışmalıdır. | Pin tıklandı; `StationDetailPanel` (380px) açıldı. Operatör ("ZES"), EPDK Sicil No ("ŞRJ/19473"), "Operatör Verisi Bekleniyor" rozeti ve kopyalama bildirimi ("İstasyon kodu: ŞRJ/19473") doğrulandı. | **GEÇTİ** |
| **UAT-05** | Konsol ve ağ hataları denetimi (TypeError & 400 Bad Request) | Tarayıcı konsolunda ve ağ trafiğinde sıfır `TypeError` ve sıfır `400 Bad Request` olmalıdır. | Canlı test boyunca konsolda **0 TypeError**, ağ trafiğinde **0 adet 400 Bad Request** ölçüldü. Hata toleransı sıfır kuralı başarıyla sağlandı. | **GEÇTİ** |
...
- **Gözlem:** API `200 OK` ile `{"type":"clusters","zoom":6,"count":81,"data":[...]}` döndü. Tarayıcı DOM'unda 106 adet kümeleme dairesi (`.cluster-marker`) çizildi. Küme sayıları 100+ için koyu lacivert (`#0F172A`), 10+ için derin mavi (`#0052A3`), küçükler için birincil mavi (`#0066CC`) tonlarında ayrıştırıldı.

### 4.3. Adım 3: İstanbul BBox ve Zoom 10-12 Uçuşu (544 Pin)
- **Etkileşim:** İstanbul kümesine (628 istasyon) tıklandı; harita `map.flyTo` ile animasyonlu olarak zoom 11 seviyesine uçtu.
- **Sorgu:** `GET /api/v1/stations?bbox=28.73922,40.95807,29.17867,41.11112&zoom=11` (1920x1080 masaüstü viewport).
- **Gözlem:** API yanıtı `{"type":"stations","count":544,"data":[...]}` olarak döndü. Harita tuvali üzerine tam **544 tekil istasyon pini** (`.station-pin`) render edildi. Ekran genişliğinde 500+ pin şartı (PO-201 ve UAT Adım 3) eksiksiz doğrulandı.
...
- **HTTP 5xx Denetimi:** Sunucu çökmesi veya 500 Server Error: **0**.
