...
const handlePrimaryAction = () => {
  ...
  // Web ortamında operatör web sitesi varsa yeni sekmede açılır;
  // İstasyon kodu panoya kopyalanır ve kullanıcıya toast verilir.
  if (import.meta.client) {
    ...
    const targetUrl = op?.deep_link_config?.web_url || `https://www.google.com/search?q=${encodeURIComponent(op.name + ' şarj istasyonu')}`;
...
<template>
  ...
  >
    <section
      v-if="isOpen && station"
      class="fixed lg:absolute left-0 top-16 bottom-0 w-full sm:w-[380px] bg-bg-surface border-r border-border-default shadow-xl z-40 flex flex-col overflow-y-auto p-5"
    ...
    >
      ...
      <div class="py-4 space-y-2.5">
        ...
        <button
          ...
          class="w-full text-center py-2 text-xs font-medium text-danger hover:underline flex items-center justify-center gap-1.5 focus-visible:outline-none"
        >
