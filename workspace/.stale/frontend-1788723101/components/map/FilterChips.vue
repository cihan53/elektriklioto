...
<template>
  <div class="relative flex items-center gap-2 overflow-x-auto py-1 scrollbar-none max-w-full">
    ...
    <button
      ...
      aria-label="Halka Açık İstasyonlar Filtresi"
    >
      <span>Halka Açık</span>
    </button>

    <!-- 3. Hızlı Şarj (DC) — Faz 1 Kilitli (Eksik Veri) -->
    <button
      type="button"
      @click="onLockedFilterClick"
      class="touch-target-min px-3.5 py-1.5 rounded-full text-sm font-medium border border-dashed border-border-strong bg-bg-subdued text-text-muted flex items-center gap-1.5 opacity-80 cursor-not-allowed flex-shrink-0"
      aria-disabled="true"
      title="Faz 1'de EPDK güç verisi bulunmadığından yakında aktifleşecektir."
    >
    ...
    <!-- 4. Boş Soketler — Faz 1 Kilitli (Eksik Veri) -->
    <button
      type="button"
      @click="onLockedFilterClick"
      class="touch-target-min px-3.5 py-1.5 rounded-full text-sm font-medium border border-dashed border-border-strong bg-bg-subdued text-text-muted flex items-center gap-1.5 opacity-80 cursor-not-allowed flex-shrink-0"
      aria-disabled="true"
      title="Faz 1'de anlık doluluk verisi bulunmadığından yakında aktifleşecektir."
    >
