...
- Tıklanabilir tüm elemanlar webde en az **44x44 CSS px** (`min-h-[44px] min-w-[44px]`), mobilde en az **48x48 pt** (`minTargetSize: 48.0`) fiziksel alana sahip olmak zorundadır. Görsel olarak küçük olan öğeler şeffaf iç boşlukla (padding) bu sınıra tamamlanır.
...
- `prefers-reduced-motion` aktif olduğunda tüm geçiş ve animasyonlar sıfırlanır: `transition: none !important; animation: none !important;`.
...
1. **Arama Çubuğu (`SearchInput`):**
   ...
   - Temizleme Butonu: Metin varken sağda 20px `X` butonu (dokunma hedefi 44x44px).
2. **Arama Sonuç Açılır Menüsü (`SearchAutocompleteDropdown`):**
   ...
   - Sonuç Maddesi: İlçe veya Operatör adı (`text-body-md`), sağda istasyon sayısı (`text-caption`, `--color-text-secondary`). Yükseklik 44px, hover zemin `--color-bg-subdued`.
...
5. **Eksik Veri Bölümü (> **VERİ YOK:** Faz 1 Kuralı):**
   ...
   - Rozet: Standart Gri Rozet (`--color-missing-bg`, metin: `--color-missing-text`):
     ...
     - Bitişik Buton (CTA): `[+ Bilgi Ekle]` (Küçük ikincil buton, yükseklik 32px, dokunma alanı 44x44px, SCR-07'yi tetikler).
...
8. **Tazelik ve Yasal EMP Beyanı (Dipnot Bloğu):**
   - Veri Kaynak Damgası: `"Veri Kaynağı: EPDK Sicil Kaydı (Eylül 2026)"` (`text-body-sm`, `--color-text-secondary`). *EPDK veri setinde istasyon bazlı güncelleme tarihi olmadığından global tohumlama referansı kullanılır.*
...
1. **Konum İzni Kapalı / GPS Devre Dışı Durumu (Zorunlu Güvenlik Kuralı):**
   - Cihazda donanım GPS'i kapalıysa veya uygulamanın konum izni "Asla" olarak engellenmişse formun en başında sarı uyarı kartı açılır:
     ...
     - Aksiyon Butonu: `[Konum İznini Aç / Ayarlar]` (Cihaz ayarlarını tetikler).
...
6. **Gönder Butonu (`ButtonPrimary`, 48px):**
...
2. **İstasyon Kart Listesi:**
   ...
   - Aksiyonlar:
     ...
     - Silme (Swipe / Buton): Kartı sola kaydırarak veya çöp kutusu ikonuyla silme (dokunma hedefi ≥ 44x44px).
     - Şarja Başla Butonu (Kompakt 36px buton, dokunma alanı 44x44px): Operatör derin bağlantısını tetikler.
...
1. **Bölüm 1: Görünüm ve Tema:**
   - Başlık: `"Görünüm"`
   - Seçenek Grubu (3 Seçenekli Segment Buton, yükseklik 44px):
     - `[ Sistem ]` | `[ Açık ]` | `[ Koyu ]`
...
2. **Bölüm 2: Depolama ve Çevrimdışı Harita:**
   ...
   - Buton (`ButtonSecondary`, 44px): `[Önbelleği Temizle]` (Onay diyaloğu açar: `"Önbellek temizlensin mi?"`).
